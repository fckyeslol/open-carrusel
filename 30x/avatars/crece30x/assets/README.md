...
Soltá acá los archivos de marca del avatar — o subilos desde la app: sección
**Assets de marca** en `http://localhost:3000/30x`. Quedan versionados en git: se
guardan para siempre y le llegan a todo el equipo con `git pull`. El sistema los
detecta solo (al subir desde la app es inmediato; a mano, al arrancar con
`npm run abrir`) — no hay que configurar nada.
