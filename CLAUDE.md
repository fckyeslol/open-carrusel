...
- `POST /api/carousels/[id]/export?format=png|pdf|html|svg&pages=1,3,5-7` — Export. `png` (default) and `svg` are one file per slide (2160px wide for PNG) and so accept exactly one page — more than one is a 400, because the loop belongs to the caller; `pdf` and `html` are one multi-slide file with whichever pages were asked for. `&pages=` is the selection ("1", "1,3", "5-7"), parsed by `src/lib/page-selection.ts` — **the same module the export menu uses**, so what the UI offers is exactly what the route accepts. Omitted ⇒ all slides (`png`/`svg` ⇒ slide 1, as before); `&slide=N` is the old alias and still works. An out-of-range page is rejected rather than clipped. The UI loops and downloads each slide separately — there is no ZIP export. The multi-slide formats go through `toPageBody()`; the PDF asks for static font instances so the text stays real text (see `slide-scope.ts` and `fonts.ts`)
